...
titleLabel = Label(
	...
	padx=20,
	pady=20,
	bg=title_label_background_color,
	fg="#fff",
)
...
text_listen = Label(
	root,
	text="System Listening Your Speech...",
		...
		try:
			...
			with sr.Microphone() as source:
				try:
					...
					speak(
						"Please select a language for translation! For selection say Bangla or English."
					)
					print("Listening...")
					text_listen.pack()
					...
					try:
						...
						if "bangla" in r.recognize_google(audio).lower():
							br = sr.Recognizer()
							with sr.Microphone() as source:
								br.adjust_for_ambient_noise(source, duration=3)
								speak(
									"Your speech will be translated in English language. Say now."
								)
								text_listen.pack()
								print("Listening...")
								...
								try:
									...
									os.remove("speak.mp3")
									destination_languages = {"English": "en"}
									...
									if count > 1:
										frame1.pack()
									system_speech(
										"Sorry could not recognize what you said. Try again."
									)
									speak(
										"Sorry could not recognize what you said. Try again."
									)
						elif "english" in er.recognize_google(audio).lower():
							er = sr.Recognizer()
							with sr.Microphone() as source:
								er.adjust_for_ambient_noise(source, duration=3)
								speak(
									"Your speech will be translated in Bangla language. Say now."
								)
								print("Listening...")
								text_listen.pack()
								...
								try:
									...
									if count > 1:
										frame2.pack()
									user_speech("You said : {}".format(text))
									destination_languages = {"Bangla": "bn"}
							...
							if count > 1:
								frame1.pack()
							system_speech(
								"Language is not registered yet. Development is in progress."
							)
							speak(
								"Language is not registered yet. Development is in progress."
							)
					...
					speak(
						"You are not connected to the internet. The program requires an active internet connection."
					)
...
info_label = Label(
	root,
	text="Press on icon to start the application",
...
trans_btn = Button(
	...
	borderwidth=0,
	pady=5,
	activebackground=application_background_color,
	command=threading_speech,
)
